...
function buttonClick(value) {
  ...
  if (calcOut.length > 15) calcOut = calcOut.substring(0, calcOut.length - 1);
...
function addSymbol(value) {
  ...
  switch (value) {
    case 'C':
      ...
      break;
    case '←':
      if (action === calcOut[calcOut.length - 1]) action = null;
    ...
    case '+':
      ...
      if (isNaN(lastSymbol)) calcOut = calcOut.substring(0, calcOut.length - 1);
...
function doCalc() {
  ...
  switch (action) {
    case '÷':
      calcOut =
        calcOut.substring(0, indexOfAction) /
    ...
    case '+':
      calcOut =
        Number(calcOut.substring(0, indexOfAction)) +
